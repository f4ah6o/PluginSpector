...
For `nv_build` and `anthropic`, the `meta_analyzer` node (the filter/enrich pass over raw findings) is upgraded to a stronger model by default — `deepseek-ai/deepseek-v4-pro` and `claude-sonnet-4-6` respectively — while every other LLM call uses the provider's base default model.
...
PluginSpector detects **81 vulnerability patterns** across 21 categories:
...
Options:
  ...
  --yara-rules-dir PATH                        Directory with additional YARA rules to load alongside built-in rules
  -V, --verbose                                Show detailed progress
  --help                                       Show this message and exit

skillspector --version  # or -v: print the installed version and exit
```
