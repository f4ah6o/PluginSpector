...
"""Shared constants for pluginspector (env-driven where applicable)."""

import os

from pluginspector.providers import get_metadata_provider
...
# ``resolve_model`` runs the waterfall: ``PLUGINSPECTOR_MODEL`` env > slot
...
# ``model = state.model or MODEL_CONFIG[ANALYZER_ID] or _PLUGINSPECTOR_DEFAULT_MODEL``).
_PLUGINSPECTOR_DEFAULT_MODEL = _provider.DEFAULT_MODEL  # type: ignore[attr-defined]
...
PLUGINSPECTOR_LOG_LEVEL = os.environ.get("PLUGINSPECTOR_LOG_LEVEL", "WARNING")
