...
from pluginspector import __version__
from pluginspector.graph import graph
from pluginspector.logging_config import get_logger, set_level
...
def version_callback(value: bool) -> None:
    ...
    if value:
        console.print(f"PluginSpector v{__version__}")
        raise typer.Exit()
...
def main(
    ...
    """
    PluginSpector - Security scanner for AI agent skills (LangGraph).
...
def scan(
    ...
    Examples:
        ...
        pluginspector scan ./my-skill/ --format json --output report.json
        pluginspector scan https://github.com/user/my-skill --no-llm

    Environment variables:

        PLUGINSPECTOR_PROVIDER  Active LLM provider: openai | anthropic |
                               nv_build | nv_inference. Defaults to the
                               NVIDIA path (nv_inference, falling back to
                               nv_build in OSS builds).
        PLUGINSPECTOR_MODEL     Override the active provider's default
                               model (applies to every analyzer slot).
        PLUGINSPECTOR_LOG_LEVEL DEBUG | INFO | WARNING | ERROR (default WARNING).

    Provider credentials (one of):

        OPENAI_API_KEY [+ OPENAI_BASE_URL]   for PLUGINSPECTOR_PROVIDER=openai
        ANTHROPIC_API_KEY                    for PLUGINSPECTOR_PROVIDER=anthropic
    ...
    try:
        ...
        tags = ["pluginspector", f"environment:{env}"]
