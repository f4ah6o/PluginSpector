...
"""Central logging configuration for the pluginspector package.

Reads ``PLUGINSPECTOR_LOG_LEVEL`` directly from the environment (default
...
_PACKAGE_LOGGER_NAME = "pluginspector"
_configured = False
...
def _configure() -> None:
    ...
    if not root.handlers:
        ...
        handler.setLevel(_level_from_string(PLUGINSPECTOR_LOG_LEVEL))
        handler.setFormatter(logging.Formatter("%(levelname)s [%(name)s] %(message)s"))
        root.addHandler(handler)
    root.setLevel(_level_from_string(PLUGINSPECTOR_LOG_LEVEL))
    _configured = True
...
def get_logger(name: str) -> logging.Logger:
    """Return a logger under the pluginspector package namespace."""
    _configure()
