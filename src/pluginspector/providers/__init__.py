...
Selection happens via the ``PLUGINSPECTOR_PROVIDER`` env var:
...
def _select_active_provider() -> ModelMetadataProvider:
    """Construct the active provider based on ``PLUGINSPECTOR_PROVIDER``."""
    name = os.environ.get("PLUGINSPECTOR_PROVIDER", "").strip().lower()
    ...
    raise ValueError(
        f"Unknown PLUGINSPECTOR_PROVIDER: {name!r}. "
