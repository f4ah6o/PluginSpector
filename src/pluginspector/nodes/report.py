...
def _build_sarif(findings: list[Finding]) -> dict[str, object]:
    ...
    sarif_log = SarifLog(
        ...
        runs=[
            SarifRun(
                tool=SarifTool(
                    driver=SarifDriver(name="pluginspector", version=pluginspector_version)
                ),
                results=results,
            )
        ],
...
def _format_terminal(
    ...
    console.print(
        Panel(
            "[bold]PluginSpector Security Report[/bold]",
            subtitle=f"v{pluginspector_version}",
        )
    )
...
def _format_markdown(
    ...
    lines.append("# PluginSpector Security Report\n")
    ...
    lines.append(f"\n*Generated by PluginSpector v{pluginspector_version}*")
    return "\n".join(lines)
